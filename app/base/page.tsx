...
import { Radar, Check, Bitcoin, Receipt, BarChart4, Castle, Building2, GrapeIcon, ArrowUp } from "lucide-react";
...
export default async function () {
    ...
    const cfuWindSpeedBlue = (cfuWindSpeed: number) => {
      ...
      return (
        ...
        <img src={svgUrl} alt="t0 wordmark" style={{ width: '450px', height: '150px' }} />
        </div>
        ...
        <Card>
          ...
          <CardContent className="text-xl">
            Miners Online: <span style={{ color: "#4ade80"}}>{roseOnline}</span> / <span style={{ color: "#4ade80"}}>{roseTotal}</span>
          </CardContent>
          <CardContent className="text-xl">
            Miners Offline: <span className="text-[#ff2f00]">{roseOffline}</span> ({roseOfflinePercentage}%)
          </CardContent>
          <CardContent className="text-xl">
            Total Hashrate: <span style={{ color: "#ff9e00"}}>{roseHashrateInPH.toFixed(2)}</span> PH/s
          </CardContent>
          <CardContent className="text-xl">
            Total Power: <span style={{ color: "#ff9e00"}}>{rosePowerDraw.toFixed(2)}</span> MW's
          </CardContent>
          <CardContent className="text-xl">
