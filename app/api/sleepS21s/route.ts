...
export async function POST(req: NextRequest) {
  const filePath = path.join(process.cwd(), 'app/api/sleepS21s/miner_ip.csv');
  const logFilePath = path.join(process.cwd(), 'app/api/sleepS21s/log.csv');
  const detailedLogFilePath = path.join(process.cwd(), 'app/api/sleepS21s/detailed_log.txt');
  ...
  fs.createReadStream(filePath)
    ...
    .on('end', async () => {
      ...
      const requestOptions = {
        ...
        body: '{"bitmain-work-mode": "1"}', // Ensure this is unformatted --- MINER MODE 0 = AWAKE --- MINER MODE 1 = SLEEP
      };
      ...
      return NextResponse.json({
        ...
        logFileUrl: '/api/sleepS21s/log' // URL to download the log file
      });
    });
